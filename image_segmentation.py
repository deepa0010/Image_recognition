...
def saveImageToDirectory(image, name, imageName):
    ...
    """
    base_path = "./output"
    directory_path = os.path.join(base_path, name)

    # Create the base directory if it does not exist
    if not os.path.exists(base_path):
        os.makedirs(base_path)

    # Create the specific directory if it does not exist
    if not os.path.exists(directory_path):
        os.makedirs(directory_path)

    # Save the image
    cv2.imwrite(os.path.join(directory_path, imageName), image)
...
def processKnownPeopleImages(path="./people/",saveLocation="./known_encodings.pickle"):
    ...
    """
    print("Processing Known people images...")
    known_encodings=[]
    known_names=[]
...
def processDatasetImages(path="./dataset/", saveLocation="./dataset_encodings.pickle"):
    ...
    ----------
    path : STRING, optional
        Path for dataset images. The default is "./dataset".
    saveLocation : STRING, optional
        Path for storing encodings. The default is "./dataset_encodings.pickle in current directory".
    ...
    for img in os.listdir(path):
        ...
        # Initialize face bounding box variables
        ...
        if len(locs) > 0:  # Ensure there are face locations to process
            for loc in locs:
                top, right, bottom, left = loc
                cv2.rectangle(image, (left, top), (right, bottom), color=(255, 0, 0), thickness=2)
            # cv2.imshow("Image", image)
